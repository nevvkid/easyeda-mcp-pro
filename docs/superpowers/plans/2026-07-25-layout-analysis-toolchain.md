...
2. **`PCB_PrimitivePoured.getAll` truncates geometry** — the *resolved* copper (antipad/thermal subtracted) serialises each fill's `path.complexPolygon` as the literal string `"[MaxDepth]"` (depth-limited serializer). So the antipad-accurate copper is un-exportable via the raw getAll; only the `Pour` **boundaries** are usable there. **FIXED 2026-07-25 — implemented `easyeda_pcb_export_pours`** (`pro` profile): the extension op composes `PCB_PrimitivePour.getAll` + `PCB_PrimitivePoured.getAll`, **flattens all geometry to plain number-array rings** (via `extractRings`, which handles `discretize()`/`{polygon}`/nested/flat forms), and returns it **base64-encoded** so the payload is a single string that can't hit the depth limit. The server writes it to a `<project>-pours.json` artifact (net + layer + priority + boundary + resolved fills + a `probe` block). Files: `easyeda-bridge-extension/src/export-operations.ts` (`exportPours`), `dispatcher.ts` (`pcb.exportPours`), `src/tools/L1_export.ts`. Both typecheck clean; needs a build + `.eext` re-import to go live. This is the data foundation for antipad-accurate plane maps ("swiss-cheese?") and the `pcb_power_audit`/`pcb_decoupling_audit` tools above.

   **Companion exports added the same batch** (mirror the same flatten-to-base64-JSON pattern; `callFirst` returns raw instances so no `normalizeValue`/`[MaxDepth]`; defensive `pick()` reads lowercase getter or `state.Capital`, with a schema probe): **`easyeda_pcb_export_pads`** (`pcb.exportPads`) — every pad's net/layer/position/size, the basis for decoupling-proximity (cap→IC power pin) and near-field coupling checks; **`easyeda_pcb_export_routing`** (`pcb.exportRouting`) — routed tracks (PCB_PrimitiveLine) + vias with nets/coords/widths, so analysis runs fully from EasyEDA primitives with no Specctra-DSN round-trip or pagination. Together (pours + pads + routing) they give the complete net-tagged board geometry from clean one-call exports. All typecheck clean; all `pro` profile.

3. **`pcb_drc_details` (F13) — deprioritised.** The one native DRC `otherClearance` overlap has shipped across multiple production runs with no field issue (owner-confirmed benign), so surfacing which two objects overlap is low value. The realtime-DRC-event approach remains documented above if the priority changes.
